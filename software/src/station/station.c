...
void station_disable_ble() { gpio_put(BLE_ENABLE_GPIO, 1); }

void station_enable_ble() { gpio_put(BLE_ENABLE_GPIO, 0); }
...
void station_init_i2c_sensor() {
    ...
    // gpio_pull_up( BME_I2C_SCL_GPIO ); // si nécéssaire ... ( car absent sur
    // le circuit ou disfonctionnement random ) gpio_pull_up( BME_I2C_SDA_GPIO
    // );
}

void station_reset_systick_counter() { station_state.systick_counter = 0; }
...
void station_main() {
    ...
    while (true) {
        ...
        printf(" - t° : %f, p : %f, h : %f \n", measures_w.temperature,
               measures_w.pressure, measures_w.humidity);
        lcd_fill_rect(10, 300, 150, 162, LCD_BLACK);
        lcd_print_double((double)(measures_w.temperature), 35, 150, LCD_YELLOW,
                         SmallFont);
        lcd_print_int((int)(measures_w.pressure), 110, 150, LCD_YELLOW,
                      SmallFont);
        lcd_print_double((double)(measures_w.humidity), 180, 150, LCD_YELLOW,
                         SmallFont);
        lcd_print_int(measures_l.ambient_light, 250, 150, LCD_YELLOW,
                      SmallFont);
        sleep_ms(1000);
    }
}
    ...
    while (true) {
        switch (station_state.mode) {
            ...
            case WEATHER_ALL_GRID:
                if (station_state.mode_initialized &&
                    station_state.systick_counter >= 1000) {
...
void station_draw_title(char* title) {
    lcd_draw_string(title, STATION_TITLE_X, STATION_TITLE_Y, LCD_LIGHT_GREEN,
                    BigFont);
}
    ...
    if (gesture_event.gesture != NO_GESTURE) {
        switch (gesture_event.gesture) {
            ...
            case MOVE_DOWN:
                ...
                break;
            case MOVE_RIGHT:
                if (station_state.current_if->swipe_right_handler != NULL)
                    station_state.current_if->swipe_right_handler();
                // lcd_clear(LCD_ORANGE);
                break;
            case MOVE_LEFT:
                if (station_state.current_if->swipe_left_handler != NULL)
                    station_state.current_if->swipe_left_handler();
                // lcd_clear(LCD_YELLOW);
                break;
            default: {
                zone_matrix_t* zm_touched =
                    display_find_zone_matrix_from_coordinates(
                        station_state.current_if, gesture_event.x,
                        gesture_event.y);
                zone_t* zone_touched = display_find_zone_from_coordinates(
                    zm_touched, gesture_event.x, gesture_event.y);
                if (gesture_event.gesture == POINT &&
                    zone_touched->point_handler != NULL)
                    zone_touched->point_handler();
                else if (gesture_event.gesture == LONG_POINT &&
                         zone_touched->long_point_handler != NULL)
