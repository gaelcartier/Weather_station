...
void test_bme280() {
    ...
    while (true) {
        ...
        printf(" - t° : %f, p : %f, h : %f \n", measures.temperature,
               measures.pressure, measures.humidity);
        sleep_ms(1000);
    }
}
...
void test_display() {
    ...
    zone_matrix_t zm;
    display_create_zone_matrix(&zm, zone_array, row, col, (point_t){0, 20},
                               (point_t){320, 220});
...
void test_i2c_nb() {
    ...
    while (1) {
        i2c_request_data(TOUCHSCREEN_I2C_INST, TOUCHSCREEN_I2C_ADDR,
                         TOUCHSCREEN_GEST_ID);
        sleep_ms(1000);
        printf("in handler() ?\n");
        if (in_i2c_irq) {
